...
test('embeds referenced local report files and returns a bare share URL', async () => {
  ...
  try {
    ...
    assert.equal(publishedTitle, '完整报告');
    assert.match(publishedMarkdown, /引用文件正文/);
...
test('renders a branded mobile share shell with dynamic metadata', () => {
  const html = renderPageHtml(
    '接口故障排查结果',
    '# 接口故障排查结果\n\n<script>alert(1)</script>\n\n[危险链接](javascript:alert(1))\n\n## 章节',
    { projectName: 'demo-project', kind: 'plain', description: '接口故障排查结果与修复建议。' },
  );
  assert.match(html, /class="page-shell"/);
  assert.match(html, /class="share-content markdown-body"/);
  assert.match(html, /<span>wecode<\/span>/);
  assert.match(html, /project-label/);
  assert.match(html, /Powered by/);
  assert.match(html, /name="robots" content="noindex, nofollow"/);
  assert.match(html, /property="og:title" content="接口故障排查结果"/);
  assert.match(html, /<title>接口故障排查结果 · demo-project · wecode<\/title>/);
  assert.doesNotMatch(html, /# 接口故障排查结果/);
  assert.match(html, /&lt;script&gt;alert\(1\)&lt;\/script&gt;/);
  assert.doesNotMatch(html, /href="javascript:/i);
  assert.doesNotMatch(html, /CODEX REPORT|分析报告|Generated by Codex/);
});

test('derives a generic title for non-report share pages', async () => {
  let publishedTitle = '';
  const pages = {
    publish: async (title: string) => {
      publishedTitle = title;
      return { title, url: 'https://share.example.test/p/generic' };
    },
  } as unknown as PagePublisher;
  const result = await renderResponse({ text: '# 会议纪要\n\n讨论结果。', presentation: 'page' }, pages);
  assert.equal(result.mode, 'page');
  assert.equal(publishedTitle, '会议纪要');
});
