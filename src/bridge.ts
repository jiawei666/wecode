...
import type { ActionResponse, PresentationMode, SessionBinding, SessionLaunchOptions, SessionSelectionItem, TurnResult } from './model.js';
import { renderResponse, PagePublisher } from './render.js';
import { buildSessionList, formatModel } from './session-display.js';
...
export class BridgeApp {
  ...
  constructor(
    ...
    this.controlAgent = new ControlAgent(config);
  ...
  private async handleInternal(message: InboundMessage): Promise<void> {
    ...
    if (expired) {
      ...
      await this.reply(userId, '【控制 Agent】控制模式已过期，当前 Codex 会话未改变。');
    }
    ...
    if (!command && /^\d+$/.test(text) && selection) {
      await this.selectSession(userId, Number(text));
      return;
    }
    if (command?.kind === 'use' && command.threadId && /^\d+$/.test(command.threadId) && selection) {
  ...
  private async handleControl(userId: string, text: string): Promise<void> {
    ...
    if (!text.trim()) {
      await this.reply(userId, '【控制 Agent·等待回复】请描述要定位、切换、新建或恢复的会话。');
      return;
    }
    ...
    await this.reply(userId, '【控制 Agent·处理中】正在理解会话操作……');
    try {
      ...
      if (result.action.action === 'ask') {
        ...
        await this.reply(userId, `【控制 Agent·等待回复】${result.action.text || '还需要一些信息。'}`, {
      ...
      if (!['list_sessions', 'status'].includes(result.action.action)) {
        this.store.clearControl(userId);
      }
    } catch (error) {
      process.stderr.write(`[control] ${errorMessage(error)}\n`);
      if (this.controlAgent.consumeInterrupted(userId)) return;
      // Keep the control state. The user can retry naturally or send /cancel.
      const latest = this.store.getControl(userId);
      if (latest) this.store.setControl(userId, { ...latest, lastActivityAt: Date.now() });
      await this.reply(userId, `【控制 Agent·未完成】${controlErrorText(error)}\n可以继续补充信息或发送 /cancel。`);
    }
  }
  ...
  private async executeAction(userId: string, action: ActionResponse): Promise<void> {
    ...
    switch (action.action) {
      case 'new_session': {
        ...
        await this.reply(userId, this.sessionCreatedText(result.binding, result.warning, '【已新建 Codex 会话】'));
        return;
      }
      case 'switch_session': {
        ...
        await this.useSession(userId, action.thread_id, launchOptions(action), action.cwd);
        return;
      }
      case 'list_sessions':
        await this.reply(userId, '【控制 Agent·查找会话】正在查找会话……');
        await this.sendSessions(userId, action.cwd ? 'here' : 'recent', false, action.cwd);
        return;
      case 'status':
        await this.sendStatus(userId);
        return;
      case 'interrupt': {
        ...
        await this.reply(userId, stopped ? '【控制 Agent】已中断当前 Codex 任务。' : '【控制 Agent】当前没有运行中的 Codex 任务。');
        return;
      }
      case 'raw_input':
        if (!action.text) throw new Error('raw_input 缺少 text');
        await this.sessions.raw(userId, action.text);
        await this.reply(userId, '【控制 Agent】已原样发送到当前 tmux TUI。');
        return;
      case 'set_note': {
        ...
        await this.reply(userId, `【控制 Agent】已记录当前会话备注：${binding.note}`);
        return;
      }
      case 'reply':
      case 'ask':
        await this.reply(userId, `【控制 Agent】${action.text || '控制操作已完成。'}`, {
          title: action.title,
  ...
  private async useSession(
    ...
    const result = await this.sessions.use(userId, threadId, requestedCwd, options);
    this.store.clearSelection(userId);
  ...
  private async sendSessions(
    ...
    const prefix = this.store.getControl(userId) ? '【控制 Agent·会话列表】\n' : '【Codex 会话列表】\n';
    await this.reply(userId, `${prefix}${display.text}`);
  }
  ...
  private async sendStatus(userId: string): Promise<void> {
    ...
    const mode = control
      ? `【控制 Agent】控制模式：${this.controlAgent.isRunning(userId) ? '处理中' : '等待你的指令'}`
      : '桥接模式：目标 Codex';
  ...
  private async stop(userId: string): Promise<void> {
    if (this.store.getControl(userId)) {
      if (await this.controlAgent.interrupt(userId)) {
        await this.reply(userId, '【控制 Agent】已发送中断，控制模式仍保留。');
      } else {
        await this.reply(userId, '【控制 Agent】当前正在等待你的回复，没有运行中的控制任务；当前 Codex 会话未中断。');
      }
      return;
    }
  ...
  private async cancel(userId: string): Promise<void> {
    if (this.store.getControl(userId)) {
      ...
      this.leaveControl(userId);
      await this.reply(userId, '【控制 Agent】已退出控制模式，当前 Codex 会话未改变。');
      return;
    }
    const binding = this.store.getBinding(userId);
    if (!binding) {
    ...
    this.store.clearSelection(userId);
    await this.reply(userId, '已停止当前任务并退出当前 Codex 绑定。原生会话仍保留，可用 /sessions 查找恢复。');
  }
  ...
  private async ensureControl(userId: string): Promise<{ sessionId?: string; startedAt: number; lastActivityAt: number }> {
    ...
    await this.reply(userId, '【控制 Agent】已进入控制模式。后续普通消息会继续交给控制 Agent；发送 /cancel 退出。');
    return created;
  }
  ...
  private async controlCatalog(): Promise<string> {
    ...
    const entries = list.slice(0, 60).map((thread) => {
      const title = thread.name || thread.preview || '未命名';
      return `- cli=codex thread_id=${thread.id} cwd=${thread.cwd || ''} title=${title} model=${thread.model || ''} reasoning=${thread.reasoningEffort || ''} serviceTier=${thread.serviceTier || 'normal'}`;
    });
    return `可恢复原生 Codex 会话（仅供定位，多个候选必须 list_sessions）：\n${entries.join('\n')}`;
  }
  ...
  private sessionCreatedText(binding: SessionBinding, warning?: string, prefix = '【已新建 Codex 会话】'): string {
    return `${prefix}\n目录：${binding.cwd}\n会话：${binding.threadId}\n模型：${formatModel(binding.model, binding.reasoningEffort, binding.fast ? 'fast' : null)}\n首条消息发送后启动 tmux TUI。${warning ? `\n⚠️ tmux 未能启动：${warning}` : ''}`;
  }

  private switchedText(binding: SessionBinding, warning?: string, prefix = '【已切换 Codex 会话】'): string {
    return `${prefix}\n目录：${binding.cwd}\n会话：${binding.threadId}\n模型：${formatModel(binding.model, binding.reasoningEffort, binding.fast ? 'fast' : null)}${warning ? `\n⚠️ ${warning}` : ''}`;
  }

  private async reply(
    userId: string,
    text: string,
    options: { title?: string; presentation?: PresentationMode; kind?: TurnResult['kind']; cwd?: string } = {},
  ): Promise<void> {
    const contextToken = this.store.get().contextTokens[userId] ?? '';
    if (!contextToken) return;
    const rendered = await renderResponse({ text, ...options }, this.pages);
...
function userFacingError(error: unknown): string {
  const message = errorMessage(error);
  if (/thread-store conflict|active writer/i.test(message)) {
