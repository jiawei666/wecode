...
export const SESSION_ROUTING_HINT = '普通消息→当前 Codex；执行中可继续发；复杂操作用“控制：……”；斜杠命令兜底。';
...
没有会话？发送“新建”或“新建 /项目目录”。
兜底：/help。`;

export const STARTUP_HINT = '首次使用发“菜单”；普通消息→当前 Codex；复杂操作用“控制：……”；斜杠命令兜底。';
