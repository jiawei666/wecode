...
这是一个面向个人本机使用的、以 Node.js/TypeScript 实现的微信 iLink 到 Codex 的桥接服务。它已经形成了清晰的第一版纵向链路：微信长轮询、用户/会话状态、控制 Agent、Codex App Server JSON-RPC、tmux 本地 TUI，以及长报告分享页。项目定位和运行说明集中在 [README.md:3](../README.md:3) 和 [README.md:7](../README.md:7)。

本地验证结果较好：`npm test` 共 13 个测试全部通过，`npm run lint`（TypeScript `--noEmit`）通过。尚不能据此判定生产链路完整，因为真实 iLink、Codex CLI/App Server、tmux 和 Cloudflare Tunnel 没有做端到端联调；`npm run build` 也未执行，以避免在只读审查中写入已有的 `dist/` 构建产物。
...
README 将项目定义为“通过微信 iLink 控制本地 Codex 交互会话”的独立 Node 服务，并明确不依赖 Obsidian（[README.md:3](../README.md:3)）。设计上，Codex 原生 thread 是会话真相，桥接层只持久化微信账号到 thread 的当前绑定，tmux 或桥接进程退出不会主动删除 Codex 历史（[README.md:21](../README.md:21)）。
...
| 会话控制 | `/new`、`/use`、`/sessions`、`/stat`、`/stop`、`/cancel` | [src/commands.ts:12](../src/commands.ts:12) |
...
| 本地 TUI | 启动/恢复 tmux-codex；`/raw` 将文字原样发送到 tmux | [src/tmux.ts:51](../src/tmux.ts:51)、[src/bridge.ts:118](../src/bridge.ts:118) |
...
| 路径 | 职责 | 关键事实 |
| --- | --- | --- |
| `src/index.ts` | 进程入口、依赖组装、登录/运行分支、轮询主循环、信号退出 | `main()` 默认 `run`，先加载配置和状态，再创建 iLink、Codex、tmux、SessionManager、BridgeApp（[src/index.ts:11](../src/index.ts:11)） |
...
| `src/sessions.ts` | 微信用户绑定、turn 累积、Codex 通知映射、tmux 生命周期 | 将 delta、plan、diff、completed 事件聚合成 `TurnResult`（[src/sessions.ts:171](../src/sessions.ts:171)） |
...
| `src/tmux.ts` / `bin/tmux-codex` | 调用 tmux、启动/恢复本地 Codex TUI | Shell 脚本检查 endpoint，必要时拉起 App Server，再执行 `codex resume`（[bin/tmux-codex:30](../bin/tmux-codex:30)） |
...
- `run`：若状态中没有 token/botId 则拒绝启动；随后实例化 `IlinkClient`、`CodexAppServer`、`TmuxManager`、`SessionManager` 和 `BridgeApp`（[src/index.ts:33](../src/index.ts:33)）。
...
3. `/cancel`、`/stop` 优先处理；随后根据是否已有控制状态、是否显式 `/ctrl`、是否为其他命令、是否已有 thread 绑定，分别进入控制 Agent、命令处理或目标 Codex（[src/bridge.ts:59](../src/bridge.ts:59)）。
4. 控制 Agent 只返回 action JSON。`BridgeApp.executeAction` 将 action 映射为新建/切换/列出/状态/中断/raw/reply 等具体行为（[src/bridge.ts:161](../src/bridge.ts:161)）。
...
`BotState` 把 iLink 登录凭据、cursor、按用户的 context token、按用户的 `SessionBinding`、控制 Agent session、去重键和最近轮询错误集中保存；`SessionBinding` 保存 thread id、cwd、tmux session、rollout 标记和活动时间（[src/model.ts:3](../src/model.ts:3)、[src/model.ts:18](../src/model.ts:18)）。
...
目标 Codex 与本地 TUI 共享同一个 App Server endpoint。`bin/tmux-codex` 在 endpoint 未就绪时启动 App Server，再通过 `codex resume <thread>` 启动 tmux；新 thread 则由 `SessionManager` 在第一次 turn 后才启动 TUI，因为首次创建时还没有可供 resume 的 rollout（[src/sessions.ts:45](../src/sessions.ts:45)、[bin/tmux-codex:35](../bin/tmux-codex:35)）。
...
| 命令 | 结果 | 覆盖/说明 |
| --- | --- | --- |
| `npm test` | 原始快照为 13 passed, 0 failed；当前源码另有配置测试 | Node test + tsx；覆盖 Codex `thread/start` sandbox 枚举、命令解析、控制 schema/action、Markdown/分片/分享页失败回退、fresh/stale session 处理和状态快速持久化（[test/render.test.ts:6](../test/render.test.ts:6)、[test/sessions.test.ts:17](../test/sessions.test.ts:17)） |
| `npm run lint` | 通过 | `tsc -p tsconfig.json --noEmit`，严格类型检查脚本定义见 [package.json:10](../package.json:10) |

测试有价值地覆盖了“新 thread 首次 turn 后再启动 tmux”和“丢失 fresh thread 时重建”两条近期易错路径（[test/sessions.test.ts:17](../test/sessions.test.ts:17)、[test/sessions.test.ts:58](../test/sessions.test.ts:58)）。渲染测试还验证了长仓库报告在没有显式 kind 时会自动走分享页，以及 cloudflared 缺失时能回退并说明原因（[test/render.test.ts:27](../test/render.test.ts:27)、[test/render.test.ts:41](../test/render.test.ts:41)）。

### 未覆盖或未执行

- 没有真实 iLink HTTP/QR/过期 cursor/sendmessage 测试；`IlinkClient` 的协议失败、重试和 context token 更新主要依赖静态审查。
- 没有真实 Codex App Server 重启/断线重连测试，也没有验证 tmux/codex/cloudflared 在目标机器上的可执行文件和权限。
...
| 优先级 | 风险/事实 | 证据与影响 | 建议 |
| --- | --- | --- | --- |
| P0 | 执行权限边界过宽 | 控制 Agent 传入 `--dangerously-bypass-approvals-and-sandbox`，目标 App Server/turn 也固定 `approvalPolicy=never`、`danger-full-access`；tmux wrapper 再次使用 bypass 参数（[src/control.ts:37](../src/control.ts:37)、[src/codex.ts:186](../src/codex.ts:186)、[bin/tmux-codex:49](../bin/tmux-codex:49)）。一旦微信入口被非预期用户使用，Codex 可读写本机广泛文件。 | 生产默认改为受限 sandbox/审批；若确需本机自动化，将权限作为显式开关，限定 workspace 根目录和可执行命令，并在高风险 action 前增加人工确认/审计。 |
...
| P2 | 集成测试和运维可观测性不足 | 现有测试主要是纯函数、假的 App Server/WebSocket 和临时目录；没有 Bridge 全链路、真实外部依赖、CI 或覆盖率门槛。 | 增加可替换的 iLink/Codex/tmux/cloudflared adapter 和 contract tests；CI 至少执行 lint/test/build，增加覆盖率、结构化日志、健康检查和依赖漏洞扫描。 |
