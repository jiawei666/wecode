...
wecode 当前采用更小的文本入口：普通消息直接进入当前 Codex；没有当前会话时自动进入会话管理模式；已有当前会话且需要管理其他会话时，说“帅哥”或“靓仔”等唤醒词进入会话管理 Agent；状态、停止、退出、帮助由 wecode 系统本地处理；不提供斜杠命令。这样不依赖 iLink 的按钮/卡片能力，也不要求手机用户在没有会话时记住额外语法。
...
没有当前会话时，普通消息会自动进入会话管理 Agent；已有当前会话且需要查找、创建、切换或恢复其他会话时，使用固定唤醒词加自然语言：
...
1. ✅ 没有当前会话时自动进入会话管理模式；已有会话时用“帅哥/靓仔 + 自然语言”显式进入管理入口。
...
当前实现已经落地：普通消息进入当前 Codex；无当前会话时自动进入会话管理 Agent；已有会话时唤醒词进入会话管理 Agent；四个中文命令不依赖会话管理 Agent。
